...
| `OPENDAQ_ENABLE_NATIVE_STREAMING_WITH_TLS` | Bool | `OFF` | Enable the TLS (`daq.nds://` and `daq.nss://`) channel of the native streaming modules | Only relevant if `OPENDAQ_ENABLE_NATIVE_STREAMING` is ON.<br>When OFF, libNativeStreaming is built without its OpenSSL dependency |
