...
class TestPropertySystem(opendaq_test.TestCase):
    ...
    def test_native_types_in_factories_nested(self):
        ...
        property_object.add_property(opendaq.ListProperty(
            'list', ['Banana',  'Apple', 'Kiwi', daq_list, ['Banana', 'Apple', 'Kiwi']], True))
        self.assertEqual(
            property_object.get_property_value('list')[3], daq_list)
        self.assertEqual(
            property_object.get_property_value('list')[4], daq_list)
