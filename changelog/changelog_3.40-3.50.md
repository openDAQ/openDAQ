...
- [#TBD](https://github.com/openDAQ/openDAQ/pull/TBD) Add the `AddDeviceScan` module manager option and the matching `General` add-device config property. When it is off, the module manager no longer scans the network for available devices before `addDevice` connects with an explicit connection string or when it resolves a connected device's streaming addresses; smart `daq://` connection strings always scan.
