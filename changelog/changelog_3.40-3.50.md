...
- [#TBD](https://github.com/openDAQ/openDAQ/pull/TBD) Add the `AddDeviceScan` module manager option and the matching `General` add-device config property. When it is off, the module manager no longer scans the network for available devices when `addDevice` connects with an explicit connection string or when it resolves a connected device's streaming addresses; smart `daq://` connection strings always scan. Independently of the option, adding a device that is not connected over a network protocol (for example `daqref://` or an audio device) no longer scans at all, since discovery results cannot apply to it; this removes about half a second from every such `addDevice`.
