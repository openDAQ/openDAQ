...
- [#TBD](https://github.com/openDAQ/openDAQ/pull/TBD) Add the `AddDeviceScan` module manager option and the matching `General` add-device config property. When it is off, `addDevice` with an explicit connection string no longer scans the network for available devices before connecting; smart `daq://` connection strings always scan.
