...
bool GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::writeLocalValue(const StringPtr& name, const BaseObjectPtr& value)
{
    ...
    else
    {
        ...
        try
        {
            shouldWrite = objPtr.getProperty(name).asPtr<IPropertyInternal>().getDefaultValueNoLock() != value;
        }
        catch(...)
        {
        }
...
template <class PropObjInterface, class... Interfaces>
ErrCode GenericPropertyObjectImpl<PropObjInterface, Interfaces...>::clearPropertyValueInternal(IString* name, bool protectedAccess, bool batch, bool isUpdating)
{
    if (name == nullptr)
        return OPENDAQ_ERR_ARGUMENT_NULL;
