...
ErrCode PropertyImpl::getPropertyType(PropertyType* type)
{
...
ErrCode PropertyImpl::getValueTypeInternal(CoreType* type, bool lock)
{
    ...
    const ErrCode errCode = daqTry([&]()
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *type = lock ? prop.getValueType() : prop.asPtr<IPropertyInternal>(true).getValueTypeNoLock();
        else
            *type = this->valueType;
    });
...
ErrCode PropertyImpl::getKeyTypeInternal(CoreType* type, bool lock)
{
    ...
    *type = ctUndefined;
    ...
    OPENDAQ_RETURN_IF_FAILED(this->getDefaultValueInternal(&defVal, lock));
...
ErrCode PropertyImpl::getDescriptionInternal(IString** description, bool lock)
{
    ...
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *description = lock ? prop.getDescription().detach() : prop.asPtr<IPropertyInternal>(true).getDescriptionNoLock().detach();
        else
...
ErrCode PropertyImpl::getUnitInternal(IUnit** unit, bool lock)
{
    ...
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *unit = lock ? prop.getUnit().detach() : prop.asPtr<IPropertyInternal>(true).getUnitNoLock().detach();
        else
...
ErrCode PropertyImpl::getMinValueInternal(INumber** min, bool lock)
{
    ...
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *min = lock ? prop.getMinValue().detach() : prop.asPtr<IPropertyInternal>(true).getMinValueNoLock().detach();
        else
...
ErrCode PropertyImpl::getMaxValueInternal(INumber** max, bool lock)
{
    ...
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *max = lock ? prop.getMaxValue().detach() : prop.asPtr<IPropertyInternal>(true).getMaxValueNoLock().detach();
        else
...
ErrCode PropertyImpl::getDefaultValueInternal(IBaseObject** value, bool lock)
{
    ...
    {
        if (const PropertyPtr prop = bindAndGetRefProp(lock); prop.assigned())
            *value = lock ? prop.getDefaultValue().detach() : prop.asPtr<IPropertyInternal>(true).getDefaultValueNoLock().detach();
        else
...
ErrCode PropertyImpl::checkContainerType(CoreType keyType, CoreType itemType)
{
    if (!defaultValue.assigned())
        return OPENDAQ_SUCCESS;
...
static bool equalsMetadataField(const BaseObjectPtr& a, const BaseObjectPtr& b)
{
    ...
    Bool eq = False;
    if (OPENDAQ_FAILED(a->equals(b, &eq)))
    {
        daqClearErrorInfo();
        return false;
    }
...
bool equalsEvalField(const TPtr& a, const TPtr& b)
{
    if (a.assigned() != b.assigned())
        return false;
...
ErrCode PropertyImpl::equals(IBaseObject* other, Bool* equal) const
{
