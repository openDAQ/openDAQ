...
// GenericRecursiveConfigLockGuard
template <typename TMutex>
GenericRecursiveConfigLockGuard<TMutex>::GenericRecursiveConfigLockGuard(const TMutex& lock, std::thread::id* threadId, int* depth)
        : id(threadId)
        , depth(depth)
        , mutex(lock)
...
template <typename TMutex>
GenericRecursiveConfigLockGuard<TMutex>::~GenericRecursiveConfigLockGuard()
{
    --(*depth);
    if (*depth == 0)
        *id = std::thread::id();
...
// LockGuard
LockGuardImpl::LockGuardImpl(IPropertyObject* owner, MutexPtr lock)
    : owner(owner)
    , mutex(lock)
    , lock(std::lock_guard(mutex))
{
...
template <typename TMutex>
RecursiveLockGuardImpl<TMutex>::RecursiveLockGuardImpl(IPropertyObject* owner, const TMutex& lock, std::thread::id* threadId, int* depth)
        : owner(owner) 
        , id(threadId)
        , depth(depth)
        , mutex(lock)
