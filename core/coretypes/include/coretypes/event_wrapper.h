...
public:
    ...
    void operator+=(Subscription&& sub)
    {
        if (!sub)
        {
            THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Must bind to a valid callable."));
        }

        if (!eventPtr.assigned())
        {
            THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Invalid or uninitialized control."));
        }
    ...
    void operator+=(const Subscription& sub)
    {
        ...
        if (!eventPtr.assigned())
        {
            THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Invalid or uninitialized event."));
        }
