...
ComponentPtr ComponentUpdateContextImpl::GetRootComponent(const ComponentPtr& curComponent)
{
...
DevicePtr ComponentUpdateContextImpl::GetDevice(const StringPtr& id, const DevicePtr& parentDevice)
{
...
std::string ComponentUpdateContextImpl::GetRootDeviceId(const std::string& id)
{
    if (id.empty())
        return id;
...
DictPtr<IString, IDeviceUpdateOptions> ComponentUpdateContextImpl::populateDeviceUpdateOptionsMapping(const UpdateParametersPtr& config)
{
...
ErrCode ComponentUpdateContextImpl::setInputPortConnection(IString* parentId, IString* portId, IString* signalId)
{
...
ErrCode ComponentUpdateContextImpl::getInputPortConnections(IString* parentId, IDict** connections)
{
    OPENDAQ_PARAM_NOT_NULL(parentId);
...
ErrCode ComponentUpdateContextImpl::removeInputPortConnection(IString* parentId)
{
    OPENDAQ_PARAM_NOT_NULL(parentId);
...
ErrCode ComponentUpdateContextImpl::setRootComponent(IComponent* baseComponent)
{
...
ErrCode ComponentUpdateContextImpl::getRootComponent(IComponent** rootComponent)
{
...
ErrCode ComponentUpdateContextImpl::getSignal(IString* parentId, IString* portId, ISignal** signal)
{
    OPENDAQ_PARAM_NOT_NULL(parentId);
    OPENDAQ_PARAM_NOT_NULL(portId);
...
ErrCode ComponentUpdateContextImpl::setSignalDependency(IString* signalId, IString* parentId)
{
    OPENDAQ_PARAM_NOT_NULL(signalId);
    OPENDAQ_PARAM_NOT_NULL(parentId);
...
ErrCode ComponentUpdateContextImpl::addDeviceRemapping(IString* originalDeviceId, IString* newDeviceId)
{
...
ErrCode ComponentUpdateContextImpl::remapInputPortConnections()
{
...
ErrCode ComponentUpdateContextImpl::getDeviceUpdateOptionsWithLocalIdOrNull(IString* localId, IDeviceUpdateOptions** options)
{
...
ErrCode ComponentUpdateContextImpl::getUpdateParameters(IUpdateParameters** updateParameters)
{
...
ErrCode ComponentUpdateContextImpl::resolveSignalDependency(IString* signalId, ISignal** signal)
{
...
ErrCode ComponentUpdateContextImpl::overrideState(IComponentUpdateContext* updateContext)
{
...
ErrCode ComponentUpdateContextImpl::getInternalState(IDict** state)
{
...
ErrCode ComponentUpdateContextImpl::serialize(ISerializer* serializer)
{
...
ErrCode ComponentUpdateContextImpl::getSerializeId(ConstCharPtr* id) const
{
...
ConstCharPtr ComponentUpdateContextImpl::SerializeId()
{
    return "ComponentUpdateContext";
}

ErrCode ComponentUpdateContextImpl::Deserialize(ISerializedObject* serialized, IBaseObject*, IFunction*, IBaseObject** obj)
{
...
StringPtr ComponentUpdateContextImpl::remapDeviceLocalIds(const std::string& componentGlobalId) const
{
    std::string output;
