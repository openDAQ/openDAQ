...
TEST_F(ComponentHolderTest, NullComponent)
{
    ...
    ASSERT_THROW(ComponentHolder(nullptr, nullptr), ArgumentNullException);
    ASSERT_THROW(ComponentHolder("id", nullptr), ArgumentNullException);
    ASSERT_THROW(ComponentHolder(nullptr, component.ptr), ArgumentNullException);
}

TEST_F(ComponentHolderTest, ComponentWithId)
{
    MockComponent::Strict component;
    const auto componentHolder = ComponentHolder("id", component.ptr);
...
TEST_F(ComponentHolderTest, SerializeDeserialize)
{
    ...
    const auto componentHolder = ComponentHolder("id", sourceComponent);
    ...
    const auto deserializeContext = ComponentDeserializeContext(ctx, parentComponent, nullptr);
