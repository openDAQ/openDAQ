...
ErrCode GenericDevice<TInterface, Interfaces...>::submitNetworkConfiguration(IString* ifaceName, IPropertyObject* config)
{
    ...
    if (!this->isRootDevice)
        return MAKE_ERROR_INFO(OPENDAQ_ERR_INVALIDSTATE, "Device must be set as root to manage network configuration.");
...
ErrCode GenericDevice<TInterface, Interfaces...>::getLog(IString** log, IString* id, Int size, Int offset)
{
    ...
    if (offset < 0)
        return MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, "Offset must be greater than or equal to 0.");

    if (size < -1)
        return MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, "Size must be greater than or equal to -1.");
