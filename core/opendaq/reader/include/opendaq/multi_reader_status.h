...
{
    // [elementType(eventPackets, Int, IEventPacket)]
    /*!
     * @brief Retrieves the dictionary of event packets from the reading process, ordered by signals.
     * @param[out] eventPackets The dictionary where the key is the signal/input port index and the value is the corresponding event packet.
     */
