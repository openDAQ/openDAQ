 ...
 * @param sampleMemSize the memory size of the binary sample.
 *
 * Binary value packet contains exactly one sample of binary type. The size of the sample is provided with
 * `sampleMemSize` argument.
 */
inline DataPacketPtr BinaryDataPacket(
    const DataPacketPtr& domainPacket,
    const DataDescriptorPtr& descriptor,
    uint64_t sampleMemSize)
{
    DataPacketPtr obj(BinaryDataPacket_Create(domainPacket, descriptor, sampleMemSize));
    return obj;
}
 ...
 * `sampleMemSize` argument. This factory does not allocate any memory for the data. Instead the memory is
...
{
    DataPacketPtr obj(BinaryDataPacketWithExternalMemory_Create(domainPacket, descriptor, sampleMemSize, data, deleter));
    return obj;
}
