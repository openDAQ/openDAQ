    ...
    size_t dataAlign)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (domainDescriptorPtr.getSampleType() != SampleType::Int64)
            return makeErrorInfo(OPENDAQ_ERR_INVALIDPARAMETER, "Domain descriptor sample type must be int64.", nullptr);
    ...
    {
        if (offsets == nullptr)
            return makeErrorInfo(OPENDAQ_ERR_INVALIDPARAMETER, "Offsets parameter must not be null for linear data rule.", nullptr);
    }
#endif
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        {
            dataSize += sampleCount[i] * sizeof(int64_t);
            dataSize = alignTo(dataSize, dataAlign);
        }
        dataSize += sampleCount[i] * rawSampleSize;
        dataSize = alignTo(dataSize, dataAlign);
        memSize += dataSize;
    }
    ...
    *reinterpret_cast<size_t*>(memPtr) = offsets != nullptr ? 1 : 0;
    ...
    const NumberPtr noOffset = Integer(0);
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (dataRuleType == DataRuleType::Explicit)
        {
            ...
            const auto packetMemSize = sizeof(int64_t) * sampleCount[i];
        ...
        size_t rawSampleSize;
        valueDescriptors[i]->getRawSampleSize(&rawSampleSize);
        const auto packetMemSize = rawSampleSize * sampleCount[i];
