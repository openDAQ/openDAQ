    ...
    switch (packet.getType())
    {
        case PacketType::Data:
        {
            if (gapCheckState == GapCheckState::uninitialized)
                THROW_OPENDAQ_EXCEPTION(InvalidStateException("No event packet received."));
...
void ConnectionImpl::initGapCheck(const EventPacketPtr& packet)
{
    ...
    {
        THROW_OPENDAQ_EXCEPTION(InvalidOperationException("Gap packets should not be inserted into connection queue from outside."));
    }
}
...
ConnectionImpl::DomainValue ConnectionImpl::numberToDomainValue(const NumberPtr& number)
{
    ...
    switch (domainSampleType)
    {
        ...
        case SampleType::Float64:
            ...
            break;
        default:
            THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Cannot convert number."));
    }
    return dv;
}
