...
StreamingSourceManager::StreamingSourceManager(const ContextPtr& context,
...
void StreamingSourceManager::coreEventCallback(ComponentPtr& sender, CoreEventArgsPtr& eventArgs)
{
...
void StreamingSourceManager::componentAdded(const ComponentPtr& sender, const CoreEventArgsPtr& eventArgs)
{
...
void StreamingSourceManager::componentUpdated(const ComponentPtr& sender, const CoreEventArgsPtr& eventArgs)
{
    auto ownerDevice = ownerDeviceRef.assigned() ? ownerDeviceRef.getRef() : nullptr;
    if (!ownerDevice.assigned())
        return;
...
void StreamingSourceManager::enableStreamingForAddedComponent(const ComponentPtr& addedComponent)
{
    auto ownerDevice = ownerDeviceRef.assigned() ? ownerDeviceRef.getRef() : nullptr;
    if (!ownerDevice.assigned())
        return;
...
void StreamingSourceManager::enableStreamingForUpdatedComponent(const ComponentPtr& updatedComponent)
{
...
void StreamingSourceManager::completeStreamingConnections(const MirroredDeviceConfigPtr& topDevice)
{
...
void StreamingSourceManager::attachStreamingsToDevice(const MirroredDeviceConfigPtr& device)
{
...
AddressInfoPtr StreamingSourceManager::findMatchingAddress(const ListPtr<IAddressInfo>& availableAddresses,
...
AddressInfoPtr StreamingSourceManager::getDeviceConnectionAddress(const DevicePtr& device)
{
...
ListPtr<IMirroredDeviceConfig> StreamingSourceManager::getAllDevicesRecursively(const MirroredDeviceConfigPtr& device)
{
