...
ErrCode GenericSyncComponentImpl<MainInterface, Interfaces...>::checkClassNameIsSyncInterface(const StringPtr& className, const TypeManagerPtr& manager) const
{
    ...
    if (auto objectClass = type.asPtrOrNull<IPropertyObjectClass>(true); objectClass.assigned())
    {
        ...
        {
            return DAQ_MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, fmt::format("Interface '{}' does not inherit from 'SyncInterfaceBase'.", className));
        }
    ...
    return DAQ_MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, fmt::format("Interface '{}' is not IPropertyObjectClass", className));
}
...
ErrCode GenericSyncComponentImpl<MainInterface, Interfaces...>::addInterface(IPropertyObject* syncInterface)
{
    ...
    {
        return DAQ_MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, "Interface name is not assigned.");
    }

    if (className == "SyncInterfaceBase")
    {
        return DAQ_MAKE_ERROR_INFO(OPENDAQ_ERR_INVALID_ARGUMENT, "Allowed adding property objects which inherits from 'SyncInterfaceBase', but not 'SyncInterfaceBase' itself.");
    }

    auto typeManager = this->context.getTypeManager();
    if (typeManager == nullptr)
    {
        return DAQ_MAKE_ERROR_INFO(OPENDAQ_ERR_ARGUMENT_NULL, "TypeManager is not assigned.");
    }
