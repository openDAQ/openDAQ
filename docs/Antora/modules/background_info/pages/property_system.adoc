...
// To extend / shorten the list, a clone of the list is required as getPropertyValue returns a frozen list. 
ListPtr<IString> list = propObj->getPropertyValue("List");
ListPtr<IString> clonedList;
for (const auto& item : list)
    clonedList.pushBack(item);

clonedList.pushBack("Pear");
propObj.setPropertyValue("List", clonedList);

// Prints "Banana", "Kiwi" and "Pear"
list = propObj->getPropertyValue("List");
for (const auto& item : list)
