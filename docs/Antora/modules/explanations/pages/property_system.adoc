...
[NOTE]
====
Non-builder factories for basic Properties of each type exist, but they only allow users to specify
...
[IMPORTANT]
====
When implementing device modules with properties that persist values in hardware (e.g., calibration data read back after a power cycle), do not use the hardware value as the default value in the property builder. Instead, use a fixed default value and set the actual hardware value separately:

[source,cpp]
----
// Correct approach for hardware-persisted values:
objPtr.addProperty(StringProperty("ChannelVPs", ""));  // fixed default
objPtr.setPropertyValue("ChannelVPs", actualValueFromHW);  // actual value from hardware
----

This ensures proper synchronization during device reconnection, as only explicitly set values (not defaults) are synchronized with the client-side mirror.
====
...
[NOTE]
====
Property types are represented by the `PropertyType` enumeration, which mirrors the `CoreType`
enumeration values (e.g. `Bool`, `Int`, `Float`, `String`, `List`, `Dict`, etc.). In addition
to the core types, `PropertyType` defines the following Property-specific types:

* *Reference* — a reference to another property on the same Property Object.
* *IndexSelection* — a selection property where the value is an integer index into a list of selectable values.
* *Selection* — a selection property where the value is the selected item itself (not an index). Supports `Int`, `Float`, and `String` value types.
* *SparseSelection* — a selection property where the value is an integer key into a dictionary of selectable values.
====
...
Value Selection Properties (``PropertyType::Selection``) differ from <<selection,Index Selection Properties>>
in a fundamental way: the Property Value holds the *selected item itself*, not an index or key.
Supported value types are `Int`, `Float`, and `String`.

The `set`/`get` Property Value calls work directly with the actual value — no additional lookup
via `getPropertySelectionValue` is needed.

:note-caption: Important
[IMPORTANT]
====
When the selection values are of type `Int`, the Property type would be ambiguous with
`IndexSelection`. To resolve this, call `setIsIntegerValueSelection(true)` on the Property
Builder. This flag tells the SDK that the integer value is the selected item, not an index.
====

.Value Selection Properties example
...
// String value selection — value IS the selected string
propObj.addProperty(
    PropertyBuilder("StringSelection")
        .setSelectionValues(List<IString>("Apple", "Banana", "Kiwi"))
        .setDefaultValue(String("Banana"))
        .build());

// Float value selection — value IS the selected float
propObj.addProperty(
    PropertyBuilder("FloatSelection")
        .setSelectionValues(List<IFloat>(1.0, 2.5, 5.0))
        .setDefaultValue(Float(2.5))
        .build());

// Integer value selection — must set setIsIntegerValueSelection(true)
// to distinguish from IndexSelection
propObj.addProperty(
    PropertyBuilder("IntSelection")
        .setSelectionValues(List<IInteger>(10, 20, 30))
        .setDefaultValue(Integer(20))
        .setIsIntegerValueSelection(true)
        .build());

// Prints "Banana" directly — get/set works with the actual value
std::cout << propObj.getPropertyValue("StringSelection") << std::endl;
propObj.setPropertyValue("StringSelection", String("Kiwi"));
std::cout << propObj.getPropertyValue("StringSelection") << std::endl;

// Prints "20" directly
std::cout << propObj.getPropertyValue("IntSelection") << std::endl;
propObj.setPropertyValue("IntSelection", Integer(30));
std::cout << propObj.getPropertyValue("IntSelection") << std::endl;
----
====
...
[NOTE]
====
Struct Properties can currently not be transmitted over OPC UA unless their Struct type name matches 
...
[NOTE]
====
Remember that IsReferenced is true for Properties that are referenced by another 
Reference Property.
====
...
openDAQ(TM) provides two distinct kinds of selection properties with different get/set semantics:

*Index-based selection* (`IndexSelection`, `SparseSelection`) — the Property Value is always an
_Integer_ representing the index (for a list) or key (for a dictionary) of the selected item.
The `getPropertyValue` / `setPropertyValue` calls work with the integer index/key.
To retrieve the actual selected item, use the dedicated `getPropertySelectionValue` getter.

:note-caption: Note
[NOTE]
====
The "dot" notation used to get/set values of nested objects does not work on the Selection
value getter function.
====

*Value-based selection* (`Selection`) — the Property Value *is* the selected item itself.
`getPropertyValue` / `setPropertyValue` return and accept the actual value (`Int`, `Float`,
or `String`). No separate selection value getter is needed.

:note-caption: Note
[NOTE]
====
For value-based selection properties with integer values, `setIsIntegerValueSelection(true)`
must be set on the builder to distinguish them from index-based integer selection properties.
====
...
+
[NOTE]
Events are currently not supported in the current version of the .NET Bindings.
====
...
The example below illustrates a possible case of error-prone behavior.
