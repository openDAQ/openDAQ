...
Readers are helpers that enable easy reading of data from xref:knowledge_base:signals.adoc[Signals] by eliminating the need for having to manually establish a xref:knowledge_base:data_path.adoc#connection[Connection] to the Signal, parsing the xref:knowledge_base:signals.adoc[Data Descriptor] info and then read xref:glossary:glossary.adoc#data_packet[Data Packets] in a correct format while still making sure that it is correct when the metadata changes.
...
As a parameter, they receive the xref:knowledge_base:signals.adoc[Signal] to read and the wanted sample-type the data and domain values should be read as. The example of <<example-constructor>> shows how a Reader for xref:knowledge_base:signals.adoc[Signal] `signal` is constructed to read _value_ data as `double` and _domain_ data as `Int64`.
...
When the requested xref:glossary:glossary.adoc#sample[Sample]-type for value or domain doesn't match with the ones produced by the xref:knowledge_base:signals.adoc[Signal], the reader attempts to convert the read data into the requested Sample type. If the data can't be converted the Reader goes into an `invalid` mode and all subsequent read operations will fail. How to resolve the invalid state is explained in <<reader_invalidation>>.
...
As the Reader receives the xref:knowledge_base:signals.adoc[Signal] as a parameter it must first establish a xref:knowledge_base:data_path.adoc#connection[Connection] to it.
...
In the case of xref:packet_reader[Packet Reader] this is the number of _packets_ ready and available in the xref:knowledge_base:data_path.adoc#connection[Connection] queue, while for xref:stream_reader[Stream Reader] this is the number of _samples_ stored in the available packets.
...
Whenever the xref:knowledge_base:signals.adoc[Signal] information changes, it sends an xref:glossary:glossary.adoc#event_packet[Event Packet] with and id of xref:knowledge_base:packets.adoc#descriptor_changed[`"SIGNAL_DESCRIPTOR_CHANGED"`].
This event contains new xref:knowledge_base:signals.adoc[Data Descriptor]s for both value and domain data.
...
[NOTE]
====
If the Reader was created with `SampleType::Undefined` the actual sample-type returned by the `getValueSampleType()` and `getDomainSampleType()` gets inferred at the first xref:knowledge_base:packets.adoc#descriptor_changed[`"DATA_DESCRIPTOR_CHANGED"`] event where the respective xref:knowledge_base:signals.adoc[Data Descriptor] is available.
Until then these calls will return `SampleType::Invalid`.

In the case of domain the xref:knowledge_base:signals.adoc[Signal] might not even have associated domain data descriptor defined, so it will be inferred at the first `readWithDomain()` call.
====
...
. *Use a transform callback*:
    * A simple function that receives raw data and the current xref:knowledge_base:signals.adoc[Data Descriptor] and outputs the transformed values back.
...
It only creates a xref:knowledge_base:data_path.adoc#connection[Connection] between the xref:knowledge_base:signals.adoc[Signal] and the Reader and gives the user the option to read xref:glossary:glossary.adoc#data_packet[Packet] after Packet or get all the currently queued ones as a list.

By itself, this does not accomplish much, but it is a great base to build upon if you need some custom specific handling that you can't achieve using any other provided reader plus you get the xref:knowledge_base:data_path.adoc#connection[Connection] queue handling for free, and since there is no other processing being done on packets, it is also as fast as it can be.

[#stream_reader]
== Stream Reader
This is the reader that will be useful in most cases.
It represents the xref:knowledge_base:data_path.adoc#connection[Connection] packet queue to the user as a continuous stream of samples and automatically advances the current read position, handles reading over xref:glossary:glossary.adoc#data_packet[Packet] boundaries and can optionally wait for the requested samples with a time-out.
...
The catch is that in openDAQ(TM) xref:knowledge_base:signals.adoc[Signal]s can have different starting points, sample rates and clocks.
Therefore, the job of a Multi Reader is to align all xref:knowledge_base:signals.adoc[Signal]s to the same starting point and on _read_ calls return values for all signals on the same domain point, usually the same time-stamp.
