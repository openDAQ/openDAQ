...
Once the simulator is started, you can connect to simulator with opendaq.
...
openDAQ(TM) can ask all loaded Modules to return information about any Device it discovers. If multiple modules return information about the same device, this means that devices supports multiple protocol, and information will be grouped. In this guide, we use
...
In the above example, we see that the simulator device is shown as `daq://openDAQ_xxx` where `openDAQ` is manufacturer name and `xxx` is a serial number.
Because simulator was discovered by multiple modules, information about simulator was grouped. To connect to the simulator, we can use any of the provided connection strings.

=== Connecting to a remote device

Once we have the list of available Devices, we can connect to one of them. In the following example, we connect to our simulator, Simulator connection string starts with `daq://openDAQ_`, so to connect to it we use the connection string that starts with `daq://openDAQ_`.
...
Reding signal last value can be simplified by using signal.last_value method. The following example shows how to read the last value of the signal.
...
To connect to simulator using the Native streaming protocol we filter out Devices which have the `"openDAQ Native Streaming"` capability.
...
# Connect the second output signal of the device to the renderer
renderer.input_ports[1].connect(device.signals[1])
