...
In this guide, we go through the motions of building and installing openDAQ(TM) from source. While this guide
uses Visual Studio as the Windows CMake generator, and Ninja as the Linux generator, you can use different
ones depending on your set-up and development environment.

Note that building openDAQ(TM) from source is meant for advanced users. We recommend for anyone new to openDAQ(TM)
to start out by downloading the binary packages, and following the
Setting up (xref:quick_start_setting_up_cpp.adoc[{cpp}]/xref:quick_start_setting_up_python.adoc[Python]) guide.
...
----
sudo apt-get update
sudo apt-get install -y git git-lfs build-essential lld cmake ninja-build mono-complete python3
