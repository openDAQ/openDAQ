...
A Module is usually a dynamically loaded library that provides a set of <<function_block>>s, xref:glossary.adoc#opendaq_opcua_server_module[Servers], and supported <<device>> types.
It can be asked to add a provided <<function_block>> / xref:glossary.adoc#opendaq_opcua_server_module[Server] to an openDAQ(TM) application, and to connect to a <<device>>.
...
* `opcua_server_module` which allows for setting up a <<opendaq_opcua_server_module,Server>> that advertises <<device>> structure.
* `native_stream_srv_module` and `ws_stream_srv_module` which allow for setting up Servers that advertise <<device>> <<signal>>s and stream <<device>> data.
...
The openDAQ(TM) xref:glossary.adoc#opendaq_opcua_client_module[OPC-UA Client Module], xref:glossary.adoc#opendaq_native_streaming_client_module[Native Streaming Client Module] and
xref:glossary.adoc#opendaq_websocket_streaming_client_module[Websocket Streaming Client Module] use a {mdns-wiki}[mDNS] query to discover all supported devices on the network.
...
and a {mdns-wiki}[mDNS] discovery service, advertising the `_streaming-ws._tcp.local.` service, with a `TXT` record entry of `caps=WS` if device is Websocket-streaming-supported.
...
The openDAQ(TM) OPC-UA Client <<module>> also can use xref:glossary.adoc#opendaq_native_streaming_client_module[openDAQ(TM) Native Streaming Client Module]
and xref:glossary.adoc#opendaq_websocket_streaming_client_module[openDAQ(TM) Websocket Streaming Client Module]
...
[#opendaq_websocket_streaming_server_module]
== Websocket Streaming Server Module

// suppress inspection "GrazieInspection"
Allows for the creation of a web-socket streaming server on an <<instance>>.
It publishes <<device>> <<signal>>s and streams <<device>> data.

[#opendaq_websocket_streaming_client_module]
== Websocket Streaming Client Module

Allows for connecting to <<device>>s running a web-socket streaming server.

The openDAQ(TM) Websocket Streaming Client Module enables its own web-socket streaming client, which connects
to the server component created by the xref:glossary.adoc#opendaq_websocket_streaming_server_module[openDAQ(TM) Websocket Streaming Server Module].
Doing so reads server signals and transfers data packets from server signals into client signals.

The openDAQ(TM) Websocket Streaming Client Module allows to read the list of all signals of
...
[#websocket_streaming_protocol]
== openDAQ(TM) Websocket streaming protocol
...
xref:glossary.adoc#opendaq_websocket_streaming_server_module[openDAQ(TM) Websocket Streaming Server]
to an xref:glossary.adoc#opendaq_websocket_streaming_client_module[openDAQ(TM) Websocket Streaming Client].

[#opendaq_native_streaming_server_module]
== Native Streaming Server Module

// suppress inspection "GrazieInspection"
Allows for the creation of a native streaming server on an <<instance>>.
It publishes <<device>> <<signal>>s and streams <<device>> data.

[#opendaq_native_streaming_client_module]
== Native Streaming Client Module

Allows for connecting to <<device>>s running a native streaming server.

The openDAQ(TM) Native Streaming Client Module enables its own native streaming client, which connects
to the server component created by the xref:glossary.adoc#opendaq_native_streaming_server_module[openDAQ(TM) Native Streaming Server Module].
Doing so reads server signals and transfers data packets from server signals into client signals.

The openDAQ(TM) Native Streaming Client Module allows to read the list of all signals of
the remote <<device>> and mirror these as a flat list as if owned by a "pseudo" local <<device>>.
This "Pseudo" <<device>> does not mirror <<channel>>s or <<function_block>>s of the remote <<device>>s,
and does not provide a mechanism to change settings of the remote <<device>>.
...
The data transfer protocol used by openDAQ(TM) to publish <<device>> available <<signal>>s and send data from <<device>>s running an
xref:glossary.adoc#opendaq_native_streaming_server_module[openDAQ(TM) Native Streaming Server]
to an xref:glossary.adoc#opendaq_native_streaming_client_module[openDAQ(TM) Native Streaming Client].

[#streaming]
== Streaming

The generalized client side mechanism for continuous and real-time receiving of data from a <<device>>.
