...
and a {mdns-wiki}[mDNS] discovery service, advertising the `_streaming-lt._tcp.local.` service, with a `TXT` record entry of `caps=WS` if device is Websocket-streaming-supported.
