...
The following examples use the Averager Function Block to show how to configure a Function Block.
It is possible to use any other bundled or custom function block. However, Input Ports, output Signals, and
properties in code snippets are specific to the Averager function block. Other function blocks may have
...
snippet for the Averager Function Block we print the current block size and then set it to 100 samples.
...
Function Block. In the case of the Averager Function Block when the BlockSize property is changed it will reconfigure the
...
Signals. Function blocks can have a variable number of Input Ports. The Averager Function Block provides one Input Port that 
