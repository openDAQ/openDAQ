...
----
====
As in preivios examples developer can implement own solution of scheduler and use it in Instance Builder. In that mode developer can discover and connect to available device using method `getAvailableDevices`. In Instance Builder developer can forcely set available device.
[tabs]
====
Cpp::
+
...
int main(int argc, const char* argv[])
{
    daq::DeviceInfoPtr defaultRootDeviceInfo = daq::DeviceInfo("connectionString");
