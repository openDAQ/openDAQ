...
`ModuleManager.AddDeviceScan` (default `true`) controls whether the module manager scans the network for available devices before `addDevice` connects with an explicit connection string and when it resolves a connected device's streaming addresses, so that server capabilities discovered on the network are merged into the device info. Smart `daq://` connection strings always scan. The same switch is available per call as the `AddDeviceScan` property of the `General` add-device config.
