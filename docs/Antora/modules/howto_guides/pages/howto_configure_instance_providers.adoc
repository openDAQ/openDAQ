...
`ModuleManager.ScanOnAdd` (default `true`) controls whether the module manager scans the network for available devices when `addDevice` is called, so that server capabilities discovered on the network are merged into the device info. With it off nothing is scanned: an explicit connection string connects without the scan, and a smart `daq://` connection string fails unless an earlier scan already listed the device. The same switch is available per call as the `ScanOnAdd` property of the `General` add-device config.
