...
`ModuleManager.AddDeviceScan` (default `true`) controls whether `addDevice` scans the network for available devices before connecting with an explicit connection string, so that server capabilities discovered on the network are merged into the device info. Smart `daq://` connection strings always scan. The same switch is available per call as the `AddDeviceScan` property of the `General` add-device config.
