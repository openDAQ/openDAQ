...
For developers requiring access to only the most recent value, the Signal and Data Packet interfaces provide a method named `getLastValue` (`get_value` in Python).

[#last_value_signal]
== Getting the last value of a Signal

When the `getLastValue` (`get_value` in Python) method is called on a Signal, it yields an `IBaseObject` containing the last value.
...
Similar is true when calling the `getLastValue` (`get_value` in Python) method on a Data Packet.
...
In this example, we cast the `lastValue` (`last_value` in Python) object into an `IRange` like so:
...
An example of something we could now do is call the methods of the underlying type and access its members such as the `getLowValue` (`get_low_value` in Python) and the `getHighValue` (`get_high_value` in Python), in this case provided by the `IRange` interface.
...
In the above example, the Data Descriptor of the Data Packet or its Signal on which we call `getLastValue` (`get_value` in Python) is:
...
In the following example, we first extract the last value via `getLastValue` (`get_value` in Python) call, then we cast the `lastValue` object into a `ComplexNumberPtr` type (not needed in Python). Lastly, call the methods of the underlying type such as the `getReal` (`get_real` in Python) and the `getImaginary` (`get_imaginary` in Python), in this case provided by the `IComplexNumber` interface.
...
[CAUTION]
====
If we want to call `getLastValue` (`get_value` in Python) on a Data Packet (as opposed to a Signal) and the underlying data type is a Struct, we must provide the optional parameter, Type Manager, which has had the custom Struct added to it.
====
...
[NOTE]
====
Dimensions of the Data Descriptor that can be set via the `setDimensions` (`set_dimensions` in Python) method for Lists in the Data Descriptor Builder and must contain exactly one dimension.
====
