...
When the `getLastValue` (`last_value` in Python) method is called on a Signal, it yields an `IBaseObject` containing the last value.
...
Similar is true when calling the `getLastValue` (`last_value` in Python) method on a Data Packet.
...
[NOTE]
====
Dimensions of the Data Descriptor that can be set via the `setDimensions` method (`set_dimensions` in Python) for Lists in the Data Descriptor Builder and must contain exactly one dimension.
====
