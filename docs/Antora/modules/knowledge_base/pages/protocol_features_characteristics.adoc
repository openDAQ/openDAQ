...
| Clients can read the current reference domain info of any signal, allowing for improved interpretation of the signal’s domain data.
^| ✅
^| 
^| 
...
| Read reference domain info
| Reference domain info is currently only supported over Native, not over the OPC UA protocol.
...
| Read reference domain info
| Reference domain info is currently only supported over Native, not over the LT Streaming protocol.
