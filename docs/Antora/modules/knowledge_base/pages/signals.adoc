...
* **Linear Rule**: The Rule parameters include a `delta` and `start` numbers. The values are calculated according to the following equation: `ReferenceDomainOffset + PacketOffset + SampleIndex * delta + start`, where the `PacketOffset` is available on the Packet carrying the data and the `SampleIndex` represents the index of the data entry being calculated within a given packet. `ReferenceDomainOffset` is only applied if available in Reference Domain Info of the Domain Signal's Data Descriptor.
