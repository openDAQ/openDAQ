...
ParsedElement parseElement(const std::string& element)
{
    ...
    // Integers are digits only; anything with leading whitespace is left to the real-number path.
    if (begin != end && !std::isspace(static_cast<unsigned char>(*begin)))
    {
        errno = 0;
        char* parsedEnd = nullptr;
        const long long integerValue = std::strtoll(begin, &parsedEnd, 10);
        if (errno == 0 && parsedEnd == end)
            return {true, static_cast<Int>(integerValue), static_cast<Float>(integerValue)};
    }
...
ParsedValues parseValues(const std::string& text)
{
    ...
    while (true)
    {
        ...
        const std::string element = text.substr(position, separator == std::string::npos ? std::string::npos : separator - position);
...
void ConstantValueFbImpl::onConnected(const InputPortPtr& port)
{
    ...
    // A value signal describes its domain separately; a signal without one is the domain itself.
    forwardsInputDomain = port.getSignal().getDomainSignal().assigned();
...
void ConstantValueFbImpl::processEventPacket(const EventPacketPtr& packet)
{
    ...
    const bool forwardedChanged = forwardsInputDomain ? domainChanged : valueChanged;
    if (!forwardedChanged)
        return;

    // Null when the input cleared its descriptor, which clears ours as well.
    domainDescriptor = forwardsInputDomain ? newDomainDescriptor : newValueDescriptor;
...
void ConstantValueFbImpl::processDomainPacket(const DataPacketPtr& packet)
{
    ...
    const auto domainPacket = forwardsInputDomain ? packet.getDomainPacket() : packet;
    if (!domainPacket.assigned() || domainPacket.getSampleCount() == 0)
        return;
