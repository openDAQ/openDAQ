    ...
    bool& end)
{
    ...
    while (i < samplesInPacket)
    {
        ...
        const double value = elementValue(sampleType, data, samplesInPacket - 1 - i);
    ...
    SignalContext& signalContext, const DataPacketPtr& packet, bool& havePrevPacket, std::unique_ptr<Polyline>& line, bool& end)
{
    ...
    {
        const double value = elementValue(sampleType, data, i + xTickOffset);
...
void RendererFbImpl::updateDomainlessValueRange(SignalContext& signalContext, const DataPacketPtr& dataPacket)
{
    ...
    const auto sampleType = packetSampleType(dataPacket, signalContext.sampleType);
    double low = std::numeric_limits<double>::max();
    double high = std::numeric_limits<double>::lowest();
    for (size_t i = 0; i < elementCount; i++)
    {
        const double value = elementValue(sampleType, dataPacket.getData(), i);
