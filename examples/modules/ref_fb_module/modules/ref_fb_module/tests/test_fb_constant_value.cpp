...
TEST_F(ConstantValueFbTest, ClearedInputDescriptorClearsOutputDomain)
{
    const auto ctx = NullContext();
    const auto fb = createFb(createModule(ctx));
    const auto domainSignal = createDomainSignal(ctx);
    ...
    fb.getInputPorts()[0].connect(domainSignal);
    const auto outputDomain = fb.getSignals()[0].getDomainSignal();
    ASSERT_TRUE(outputDomain.getDescriptor().assigned());

    domainSignal.setDescriptor(nullptr);
    ASSERT_FALSE(outputDomain.getDescriptor().assigned());

    const auto restored = createDomainSignal(ctx).getDescriptor();
    domainSignal.setDescriptor(restored);
    ASSERT_EQ(outputDomain.getDescriptor(), restored);
}

TEST_F(ConstantValueFbTest, ValueDescriptorChangeLeavesForwardedDomainAlone)
{
    const auto ctx = NullContext();
    const auto fb = createFb(createModule(ctx));
    ...
    fb.getInputPorts()[0].connect(valueSignal);
    const auto outputDomain = fb.getSignals()[0].getDomainSignal();
    ASSERT_EQ(outputDomain.getDescriptor(), domainSignal.getDescriptor());

    // Only the domain is forwarded, so changes to the value signal's own descriptor, including clearing it, are ignored.
    valueSignal.setDescriptor(DataDescriptorBuilder().setSampleType(SampleType::Int32).setRule(ExplicitDataRule()).build());
    ASSERT_EQ(outputDomain.getDescriptor(), domainSignal.getDescriptor());

    valueSignal.setDescriptor(nullptr);
    ASSERT_EQ(outputDomain.getDescriptor(), domainSignal.getDescriptor());
}

TEST_F(ConstantValueFbTest, ForwardedDomainFollowsInputDomainDescriptor)
{
    const auto ctx = NullContext();
    const auto fb = createFb(createModule(ctx));
    ...
    const auto finer = DataDescriptorBuilderCopy(domainSignal.getDescriptor()).setTickResolution(Ratio(1, 1000000)).build();
    domainSignal.setDescriptor(finer);
    ASSERT_EQ(outputDomain.getDescriptor(), finer);
