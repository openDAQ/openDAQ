...
TEST_F(WsStreamingServerModuleTest, VersionCorrect)
{
    ...
    ASSERT_EQ(version.getMajor(), NEWER_WS_STREAM_SRV_MODULE_MAJOR_VERSION);
    ASSERT_EQ(version.getMinor(), NEWER_WS_STREAM_SRV_MODULE_MINOR_VERSION);
...
TEST_F(WsStreamingServerModuleTest, GetAvailableComponentTypes)
{
    ...
    ASSERT_TRUE(serverTypes.hasKey("OpenDAQNewerLTStreaming"));
...
TEST_F(WsStreamingServerModuleTest, ServerConfig)
{
    ...
    ASSERT_TRUE(serverTypes.hasKey("OpenDAQNewerLTStreaming"));
    auto config = serverTypes.get("OpenDAQNewerLTStreaming").createDefaultConfig();
