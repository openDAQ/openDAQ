...
void PowerReaderFbImpl::onDataReceived()
{
    ...
    const auto domainData = std::make_unique<int64_t[]>(cnt);
    std::array<double*, 2> data{voltageData.get(), currentData.get()};

    const MultiReaderStatusPtr status = reader.readWithDomain(data.data(), domainData.get(), &cnt);

    if (cnt > 0)
    {
        const auto powerDomainPacket = DataPacket(powerDomainSignal.getDescriptor(), cnt, domainData[0]);
