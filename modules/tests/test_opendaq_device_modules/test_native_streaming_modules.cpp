...
TEST_F(NativeStreamingModulesTest, GetRemoteDeviceObjects)
{
    ...
    for (size_t i = 0; i < serverSignals.getCount(); ++i)
    {
        ...
        ASSERT_EQ(serverSignals[i].getPropertyValue("Name"), clientSignals[i].getPropertyValue("Name"));
        ASSERT_EQ(serverSignals[i].getPropertyValue("Description"), clientSignals[i].getPropertyValue("Description"));
    }
...
TEST_F(NativeStreamingModulesTest, Reconnect)
{
    SKIP_TEST_MAC_CI;
    auto server = CreateServerInstance();
    auto client = CreateClientInstance();

    auto clientSignalsBeforeDisconnection = client.getSignalsRecursive();

    auto signal = clientSignalsBeforeDisconnection[0].template asPtr<IMirroredSignalConfig>();
    auto domainSignal = signal.getDomainSignal().template asPtr<IMirroredSignalConfig>();
    ...
    // test there is no more data to read after disconnection
    {
        std::this_thread::sleep_for(100ms);
        double samples[1000];
        daq::SizeT count = 1000;
    ...
    // wait for new subscribe ack before further tests
    ...
    auto clientSignalsAfterReconnection = client.getSignalsRecursive();
    ...
    for (size_t i = 0; i < serverSignals.getCount(); ++i)
    {
        ...
        ASSERT_EQ(serverSignals[i].getPropertyValue("Name"),
                  clientSignalsAfterReconnection[i].getPropertyValue("Name"));
        ASSERT_EQ(serverSignals[i].getPropertyValue("Description"),
                  clientSignalsAfterReconnection[i].getPropertyValue("Description"));
