...
TEST_P(StreamingTest, ChangedDataDescriptorBeforeSubscribe)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            // In websocket streaming-lt, a single event packet read on the server side
            // results in the generation of 2 event packets on the client side.
            // The event packet is split and transmitted via signal meta-information, separately for value signals
            // and domain signals. On the client side, one packet is generated due to meta-info changes for the value signal,
            // and another due to changes in the domain signal meta-info. Consequently, we observe 3 packets on the client side:
            // the first generated by the configuration protocol, and the subsequent two by streaming.
            // Notably, the first streaming event packet, which only contains changes for either the value or domain signal,
            // is distinct from the packet generated by the configuration protocol.
            size_t eventPacketsCnt = std::get<1>(GetParam()) == "daq.wss" ? 3 : 1;

            auto clientReceivedPackets = tryReadPackets(clientReader, packetsToRead + eventPacketsCnt);
            ASSERT_EQ(clientReceivedPackets.getCount(), packetsToRead + eventPacketsCnt);
