                                                        ...
                                                        const std::vector<Permission>& requiredPermissions)
{
    ...
    for (const auto permission : requiredPermissions)
    {
        if (!permissionManager.isAuthorized(user, permission))
            THROW_OPENDAQ_EXCEPTION(AccessDeniedException());
    }
}
...
inline void ConfigServerAccessControl::protectLockedComponent(const ComponentPtr& component)
{
    ...
    if (device.assigned() && device.isLocked())
        THROW_OPENDAQ_EXCEPTION(DeviceLockedException());
}
...
inline void ConfigServerAccessControl::protectViewOnlyConnection(ClientType connectionType)
{
    if (connectionType == ClientType::ViewOnly)
        THROW_OPENDAQ_EXCEPTION(AccessDeniedException("Operation is not avilable under view-only connection"));
}
