...
static std::unordered_map<IntfID, std::function<OpcUaVariant(const BaseObjectPtr&, const UA_DataType*, const ContextPtr&)>> idToVariantMap{
    {INumber::Id,
...
static std::unordered_map<OpcUaNodeId, std::function<BaseObjectPtr(const OpcUaVariant&, const ContextPtr& context)>> uaTypeToDaqObject{
    ...
    {OpcUaNodeId(NAMESPACE_DAQBSP, UA_DAQBSPID_FUNCTIONBLOCKINFOSTRUCTURE),
     [](const OpcUaVariant& var, const ContextPtr& context) { return VariantConverter<IFunctionBlockType>::ToDaqObject(var, context); }},
...
static std::unordered_map<OpcUaNodeId, std::function<ListPtr<IBaseObject>(const OpcUaVariant&, const ContextPtr& context)>> uaTypeToDaqList{
    {OpcUaNodeId(0, UA_NS0ID_BOOLEAN),
