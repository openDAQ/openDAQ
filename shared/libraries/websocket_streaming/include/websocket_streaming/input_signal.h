                                      ...
                                      const nlohmann::json& constRuleStartValueMeta)
{
    ...
    if (isTimeSignal)
    {
        if (dataRuleType == daq::DataRuleType::Linear)
            ...
            THROW_OPENDAQ_EXCEPTION(ConversionFailedException("Unsupported input domain signal rule"));
    }
    else
    {
        if (dataRuleType == daq::DataRuleType::Explicit)
            ...
            THROW_OPENDAQ_EXCEPTION(ConversionFailedException("Unsupported input data signal rule"));
    }
}
