...
void OutputValueSignalBase::writeValueDescriptorChanges(const DataDescriptorPtr& valueDescriptor)
{
    ...
    {
        THROW_OPENDAQ_EXCEPTION(ConversionFailedException("Unassigned value descriptor"));
    }
}
...
void OutputValueSignalBase::writeDomainDescriptorChanges(const DataDescriptorPtr& domainDescriptor)
{
    ...
    {
        THROW_OPENDAQ_EXCEPTION(ConversionFailedException("Unassigned domain descriptor"));
    }
}
...
{
    THROW_OPENDAQ_EXCEPTION(InvalidOperationException("Streaming-lt: explicit streaming of domain signals is not supported"));
}
    ...
    daq::streaming_protocol::LogCallback logCb)
{
    ...
    if (daqSampleType != daq::SampleType::Int64 &&
        ...
        THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Unsupported domain signal sample type - only 64bit integer types are supported"));

    auto dataRule = descriptor.getRule();
    if (dataRule.getType() != DataRuleType::Linear)
        THROW_OPENDAQ_EXCEPTION(InvalidParameterException("Invalid domain signal data rule - linear rule only is supported"));
    ...
    daq::streaming_protocol::LogCallback logCb)
{
    ...
    switch (sampleType)
    {
        ...
        default:
            THROW_OPENDAQ_EXCEPTION(InvalidTypeException("Unsupported data signal sample type - only real numeric types are supported"));
    }
