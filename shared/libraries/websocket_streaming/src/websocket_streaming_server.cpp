...
void WebsocketStreamingServer::start()
{
    ...
    ServerCapabilityPtr serverCapability = ServerStreamingCapability("daq.wss");
...
{
    this->device.getInfo().asPtr<IDeviceInfoInternal>().removeServerCapability(String("daq.wss"));
    stopInternal();
}
