...
{
    auto client = std::make_shared<StreamingClient>(NullContext(), "daq.ws://127.0.0.1");
    ...
    client = std::make_shared<StreamingClient>(NullContext(), "daq.ws://localhost/path/other");
    ASSERT_EQ(client->getPort(), daq::streaming_protocol::WEBSOCKET_LISTENING_PORT);
    ASSERT_EQ(client->getHost(), "localhost");
    ASSERT_EQ(client->getTarget(), "/path/other");

    client = std::make_shared<StreamingClient>(NullContext(), "daq.ws://localhost:3000/path/other");
